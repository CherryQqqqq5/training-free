...
RESULT = ROOT / "abhe_v0_miss_param_residual_dev_smoke_result.json"
FAILURE = ROOT / "abhe_v0_miss_param_residual_dev_smoke_failure.json"
...
def _failure(arm: str, blockers: List[str]) -> Dict[str, Any]:
    data = {
        "artifact_kind": "abhe_v0_miss_param_residual_dev_smoke_failure",
        "schema_version": "abhe_v0_miss_param_residual_dev_smoke_failure_v0",
        "arm": arm,
...
def _adapter_for_arm(arm: str, selected_hash: str) -> Path:
    ...
    slot_v1 = arm == "slot_recovery_v1"
    adapter = {
        "artifact_kind": "abhe_v0_miss_param_runtime_candidate_adapter",
        "schema_version": "abhe_v0_miss_param_runtime_candidate_adapter_v0",
        "arm": arm,
        ...
        "runtime_projection": [
            {
                ...
                "missing_param_slot_recovery_controller_v1": slot_v1,
    ...
    out = ROOT / f"abhe_v0_miss_param_runtime_candidate_adapter_{arm}.json"
    _write(out, adapter)
    return out
...
def _write_arm(arm: str, selected_hash: str, case_count: int, ids_by_category: Dict[str, List[str]], entry_by_category: Dict[str, str], status_by_category: Dict[str, Dict[str, Any]], latency: float) -> Dict[str, Any]:
    ...
    artifact = {
        "artifact_kind": "abhe_v0_miss_param_residual_dev_smoke_arm_compact",
        "schema_version": "abhe_v0_miss_param_residual_dev_smoke_arm_compact_v0",
        "arm": arm,
    ...
    _write(ROOT / f"abhe_v0_miss_param_residual_dev_smoke_{arm}_arm_compact.json", artifact)
    return artifact
...
def _write_result(selected_hash: str) -> None:
    ...
    for arm in ARMS:
        ...
        if path.exists():
            arms[arm] = _load(path)
    result = {
        "artifact_kind": "abhe_v0_miss_param_residual_dev_smoke_result",
        "schema_version": "abhe_v0_miss_param_residual_dev_smoke_result_v0",
        "bounded_dev_smoke_only": True,
        "compact_only": True,
        "selected_case_ids_hash": selected_hash,
...
def execute_arm(arm: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"report_scope": "abhe_v0_miss_param_residual_dev_smoke_execute", "arm": arm, "execution_started": True, "provider_calls_made": True, "bfcl_generate_called": True, "bfcl_evaluate_called": True, "scorer_called": True, "compact_only": True, "raw_material_absent": True, "performance_evidence": False, "arm_compact": compact, "blockers": []}
    except Exception as exc:
