...
def _case_result_from_trace(trace: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "next_tool_args_match_binding_normalized": validation.get("next_tool_args_match_binding_normalized"),
        "normalized_arg_binding_validation": validation.get("normalized_arg_binding_validation") or {},
        ...
        "next_tool_final_args_match_binding_normalized": validation.get(
            "next_tool_final_args_match_binding_normalized"
        ),
        "final_normalized_arg_binding_validation": validation.get("final_normalized_arg_binding_validation") or {},
...
def summarize_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    family_summary: dict[str, dict[str, Any]] = defaultdict(
        lambda: {
            ...
            "arg_emitted": 0,
            "arg_binding_match": 0,
            "normalized_arg_binding_match": 0,
            "final_arg_binding_match": 0,
            "final_normalized_arg_binding_match": 0,
    ...
    for item in results:
        ...
        family["normalized_arg_binding_match"] += int(
            item.get("next_tool_args_match_binding_normalized") is True
        )
        family["final_arg_binding_match"] += int(item.get("next_tool_final_args_match_binding") is True)
        family["final_normalized_arg_binding_match"] += int(
            item.get("next_tool_final_args_match_binding_normalized") is True
        )
    ...
    summary = {
        ...
        "normalized_arg_binding_match_count": sum(
            int(item.get("next_tool_args_match_binding_normalized") is True) for item in results
        ),
        "final_arg_binding_match_count": sum(
            int(item.get("next_tool_final_args_match_binding") is True) for item in results
        ),
        "final_normalized_arg_binding_match_count": sum(
            int(item.get("next_tool_final_args_match_binding_normalized") is True) for item in results
        ),
...
def render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Normalized arg binding match: `{summary['normalized_arg_binding_match_count']}`",
        f"- Final arg binding match: `{summary['final_arg_binding_match_count']}`",
        f"- Final normalized arg binding match: `{summary['final_normalized_arg_binding_match_count']}`",
    ...
    lines.extend(["", "## Family Summary", "", "| Family | Total | Expected Activate | Activated | Emitted | Tool Match | Arg Emitted | Raw Strict Arg Match | Raw Normalized Arg Match | Final Strict Arg Match | Final Normalized Arg Match | Stop FP |", "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"])
    for family, row in summary["family_summary"].items():
        lines.append(
            ...
            f"{row['arg_binding_match']} | {row['normalized_arg_binding_match']} | "
            f"{row['final_arg_binding_match']} | {row['final_normalized_arg_binding_match']} | "
            f"{row['stop_allowed_false_positive']} |"
        )
    lines.extend(["", "## Blocked Reasons", "", "| Reason | Count |", "| --- | ---: |"])
    for reason, count in summary["blocked_reason_distribution"].items():
        lines.append(f"| {reason} | {count} |")
    lines.extend(["", "## Cases", "", "| Case | Family | Activated | Emitted | Tool Match | Raw Strict Arg Match | Raw Normalized Arg Match | Final Strict Arg Match | Final Normalized Arg Match | Status |", "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"])
    for result in summary["results"]:
        lines.append(
            ...
            f"{int(result.get('next_tool_args_match_binding_normalized') is True)} | "
            f"{int(result.get('next_tool_final_args_match_binding') is True)} | "
            f"{int(result.get('next_tool_final_args_match_binding_normalized') is True)} | "
            f"{result.get('status_code') or '-'} |"
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(json.dumps({key: summary[key] for key in [
        ...
        "normalized_arg_binding_match_count",
        "final_arg_binding_match_count",
        "final_normalized_arg_binding_match_count",
