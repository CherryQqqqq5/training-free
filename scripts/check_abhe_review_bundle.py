...
def check(output: Path = DEFAULT_OUTPUT) -> Dict[str, Any]:
    bundle = build_bundle()
    if output.exists():
        persisted = _load_json(output)
        persisted_blockers = validate_bundle(persisted)
        if persisted_blockers:
            bundle.setdefault("blockers", []).extend("persisted:%s" % blocker for blocker in persisted_blockers)
            bundle["abhe_review_bundle_ready"] = False
    return bundle
