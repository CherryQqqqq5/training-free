...
# Requires: conda env with grc + bfcl-eval, OpenRouter credentials, and (if needed) your BFCL model alias.
set -euo pipefail
...
for CAT in "${CATEGORIES[@]}"; do
  ...
  if ! grc compile \
    ...
    echo "compile returned non-zero for ${CAT}; inspecting compile_status.json"
  fi
  ...
  if [[ "${COMPILE_STATUS}" != "actionable_patch" ]]; then
    ...
    echo "skipped patch benchmark for ${CAT}: ${COMPILE_STATUS}"
    continue
  fi
