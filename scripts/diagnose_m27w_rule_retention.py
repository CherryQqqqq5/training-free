...
def main() -> int:
    ...
    if args.compact:
        print(json.dumps({k: report.get(k) for k in ["holdout_manifest_ready", "offline_u_v_readiness_passed", "dev_scorer_net_case_gain", "decision_distribution", "regression_case_count", "m27y_scorer_feedback_ready", "scorer_feedback_covers_regressions", "pattern_feedback_effective_for_regressions", "m27w_rule_retention_passed"]}, indent=2, sort_keys=True))
    return 0
