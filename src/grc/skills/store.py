...
def load_skill(path: Path) -> tuple[Skill | None, list[str]]:
    ...
    return Skill(
        ...
        trigger_priority=int(data.get("trigger_priority") or 9999),
        max_injection_tokens=int(data.get("max_injection_tokens") or 0),
        conflicts_with=tuple(str(v) for v in data.get("conflicts_with") or []),
        requires_schema=bool(data.get("requires_schema")),
        requires_current_turn=bool(data.get("requires_current_turn")),
        forbidden_sources=tuple(str(v) for v in data.get("forbidden_sources") or []),
        evaluation_status=str(data.get("evaluation_status") or ""),
