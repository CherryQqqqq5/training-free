...
- Trace count: `151`
- Candidate count: `48`
- Candidate operation distribution: `{'retrieve': 48}`
- Candidate category distribution: `{'memory_kv': 24, 'memory_rec_sum': 24}`
- Rejection reasons: `{'candidate_ready': 48, 'memory_postcondition_already_satisfied': 66, 'no_memory_operation_intent': 28, 'no_memory_tools_available': 9}`

## Theory Prior

When durable memory facts are requested or supplied and memory tools are schema-available, the agent should not answer from prose before an observable retrieve/write memory postcondition is satisfied.
