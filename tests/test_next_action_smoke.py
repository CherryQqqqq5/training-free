...
class NextActionSmokeTests(unittest.TestCase):
    ...
    def test_compiler_generated_report_counts_arg_binding_matches(self) -> None:
        ...
        self.assertEqual(summary["case_count"], 20)
        self.assertEqual(summary["passed_count"], 15)
        self.assertEqual(summary["action_candidate_count"], 8)
        self.assertGreaterEqual(summary["arg_binding_present_count"], 8)
        self.assertEqual(summary["selected_tool_match_count"], 10)
        self.assertGreaterEqual(summary["arg_binding_match_count"], 8)
        self.assertEqual(summary["blocked_reason_distribution"].get("action_candidate_guard_rejected", 0), 3)
        self.assertEqual(summary["stop_allowed_false_positive_count"], 0)
