...
class NextActionSmokeTests(unittest.TestCase):
    ...
    def test_compiler_generated_report_counts_arg_binding_matches(self) -> None:
        ...
        self.assertEqual(summary["action_candidate_count"], 9)
        self.assertGreaterEqual(summary["arg_binding_present_count"], 9)
        self.assertEqual(summary["selected_tool_match_count"], 9)
        self.assertGreaterEqual(summary["arg_binding_match_count"], 9)
        self.assertEqual(summary["blocked_reason_distribution"].get("action_candidate_guard_rejected", 0), 6)
        self.assertEqual(summary["stop_allowed_false_positive_count"], 0)
